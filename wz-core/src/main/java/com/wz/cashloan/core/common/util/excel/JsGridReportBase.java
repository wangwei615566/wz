package com.wz.cashloan.core.common.util.excel;
	...
	public void exportExcel(List<Object> list,String title,String[] hearders,String[] fields,String operatorName) throws Exception{
		...
		if (listResult.size()>0) {
			...
			for (int i = 0; i < listResult.size(); i++) {
				td = ExcelUtil.createTableData(listResult.get(i), ExcelUtil.createTableHeader(hearders,grouped), fields);
				td.setSheetTitle(title + "_("+(i+1)+")");
				tableDataLst.add(td);
			}
			exportToExcel(excelTitle, operatorName, tableDataLst);
		}else{
			td = ExcelUtil.createTableData(list, ExcelUtil.createTableHeader(hearders,grouped), fields);
