...
public class ServletUtils {
	...
	public static void writeNoFormatDateToResponse(HttpServletResponse response, Map<? extends Object, Object> res){
		response.addHeader(CONTENT_TYPE, JS_TYPE);
		response.setContentType("application/json");
		response.setCharacterEncoding(CODE_UTF8);
		OutputStreamWriter out = null;
		...
		logger.info(JsonUtil.toString(res));
		JsonUtil.writeNoFormatDate(out, res);
	}
	
	public static void writeXmlResponse(HttpServletResponse response, Map<String, String> res) {
		...
		String resStr = XmlUtils.toXml(res);
		try {
			out.append(resStr);
			out.flush();
		} catch (IOException e) {
