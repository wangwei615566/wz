...
//	@RequestMapping("/modules/system/sconfig/saveOrUpdateRoleMenus.htm")
	...
	public void saveOrUpdate(HttpServletRequest request, HttpServletResponse response,
		...
		List<Long> meuns = sysPermService.findMeuns(permIds);
		List<Long> menuIds = JSONObject.parseArray(menus, Long.class);
		for (Long l:meuns
			 ) {
			if (!menuIds.contains(l)) menuIds.add(l);
		}
					...
					for (Map<String, Object> perm : list) {
						if ((StringUtil.isNull(perm.get("menuId")).equals(StringUtil.isNull(menuChild.get("value"))))) {
							...
							for (SysRolePerm rolePerm:sysRolePerms
								...
								if (StringUtil.isNull(rolePerm.getPermId()).equals(StringUtil.isNull(perm.get("id")))){
									flag = true;
								}else {
									checked = 1;
								}
							}
							if (flag && checked == 1)menuChild.put("checked",0);//当权限框有选中但是没有全选时,不能全部勾选
							perm.put("isRolePerm",flag);
							if (perm.get("isRolePerm") == null) perm.put("isRolePerm",false);
							if (null == menuPerms) {
